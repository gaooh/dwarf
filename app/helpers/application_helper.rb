...
module ApplicationHelper
  ...
  def link_star_to(todo)
    "<a href=\"#{APP_CONFIG[:url]}/todos/show/#{todo.id}\" >#{todo.todo}</a>"
  end
  
  def done_length
    "#{calc_task_length(@task_rate)}px"
  end
  
  def undo_length
    "#{330 - calc_task_length(@task_rate)}px"
  end
  ...
  def nickname
    session[:user_name] = User.find(session[:user]).nickname if session[:user_name].nil? 
    session[:user_name]
  end
  ...
  private
    def calc_task_length(value)
      (330.to_f * 1/100 * value).round
    end
end
